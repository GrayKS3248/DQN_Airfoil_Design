...
class Vortex_Panel_Solver():
    def __init__(self, max_num_steps, n_panels_per_surface, v_inf_test_points, alpha_test_points, cl_test_points, cdp_test_points, cm4c_test_points):
        ...
        self.num_j = np.linspace(1,0,self.precision)
        self.num_jp1 = np.linspace(0,1,self.precision)
    ...
    def solve_integral(self, pj, pjp1, cp, cp_normal):
        ...
        # solve integrals
        vx_prime_j = np.trapz(ry * self.num_j / den, x=s_norm)
        vx_prime_jp1 = np.trapz(ry * self.num_jp1 / den, x=s_norm)
        vy_prime_j = np.trapz(minus_rx * self.num_j / den, x=s_norm)
        vy_prime_jp1 = np.trapz(minus_rx * self.num_jp1 / den, x=s_norm)
